...
export const Route = createFileRoute("/student/flagged")({ component: StudentFlagged });

async function getToken() {
  return (await supabase.auth.getSession()).data.session?.access_token ?? null;
}

function StudentFlagged() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="These were flagged by Buffr's risk monitoring."
      />
      {loading
        ? <p className="text-sm text-muted-foreground">Loading…</p>
        : <TransactionsTable transactions={transactions} flaggedOnly />}
    </div>
  );
}
